    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            String sql = "SELECT p.product_id, p.seller_id, p.category_id, p.title, p.product_desc, " +
                    ...
                    "p.image_urls, p.tags, p.pickup_point_id, p.publish_status " +
                    "FROM products p " +
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    product.setPickupPointId(rs.getLong("pickup_point_id"));
                    if (rs.wasNull()) product.setPickupPointId(null);
            ...
            try (PreparedStatement ps3 = conn.prepareStatement(ppSql);
                 ResultSet rs3 = ps3.executeQuery()) {
                List<Map<String, Object>> pickupPoints = new ArrayList<>();
                while (rs3.next()) {
                    Map<String, Object> map = new HashMap<>();
                    map.put("pickupPointId", rs3.getLong("pickup_point_id"));
                    map.put("pointName", rs3.getString("point_name"));
                    map.put("campusArea", rs3.getString("campus_area"));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String updateSql = "UPDATE products SET " +
                    ...
                    "condition_level=?, cover_image_url=?, image_urls=?, tags=?, pickup_point_id=?, updated_at=NOW() " +
                    "WHERE product_id=? AND seller_id=? AND IFNULL(is_deleted,0)=0";
            ...
            try (PreparedStatement ps = conn.prepareStatement(updateSql)) {
                ...
                if (pickupPointIdStr != null && !pickupPointIdStr.trim().isEmpty()) {
                    try {
                        ps.setLong(10, Long.parseLong(pickupPointIdStr.trim()));
                    } catch (NumberFormatException e) {
                ...
                ps.setInt(12, loginUser.getUserId());
